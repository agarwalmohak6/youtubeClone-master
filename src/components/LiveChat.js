    ...
    <div className="border border-black w-full p-2 m-1 rounded-lg shadow-lg">
      <div className=" h-[380px] flex flex-col-reverse overflow-y-scroll ">
        {ChatMessages.map((chat, index) => (
          <ChatMessage key={index} name={chat.name} message={chat.messages} />
        ))}
      </div>
      ...
      >
        <input
          type="text"
          className="border border-black rounded-sm shadow-lg ml-4 px-2"
          value={liveMessage}
          onChange={(e) => {
        ...
        <button
          className="px-2 bg-green-100 font-bold py-1 mx-2"
          onClick={() => {
            dispatch(
              addMessage({
                name: "mohak",
